...
# 6. As stated in Cloudflare API doc, the fields returned may be changed. For
#    the benefit of parser devlopment, fields are hard-coded here but can be 
#    changed by modifying the values in paramter fields. Full list of
#    fields (as of 19-Jun-2020) can be found in $fields_all.
# 7. Reduce timeRangeMins if you encounter time-out error.
# 8. Logs can be found in ./log folder and logs are kept for 7 days only.
# 9. Only 1 running instance allowed at all times. Delete pid.lock if the previous
...
Function Download-Data {
    ...
    Write-Log -message ("REST API URL (fields excluded): " + $url)
...
$dataFolder         = "$PSScriptRoot\data\$todayStr"
$logfile     = $scriptLogFolder+$todayStr+".log"
$posFile     = "$PSScriptRoot\cf.pos"
$posFileTimeFormat  = "yyyy-MM-ddTHH:mm:ssZ"
$dataFileTFormat    = "yyyyMMddHHmmss"
$lockFile           = "$PSScriptRoot\pid.lock"

$zoneUrl            = "https://api.cloudflare.com/client/v4/zones/$zoneID/logs/received?timestamps=unix"
